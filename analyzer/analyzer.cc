...
static inline bool excludeProcess(ProcessGroup aProcessGroup,
				  const char name[], const char args[],
				  const char user[], const char group[])
{
  std::vector<RegExpRule>::iterator regExps;

  for (regExps = aProcessGroup.excludeProcs.begin();
       regExps != aProcessGroup.excludeProcs.end();
       regExps++)
    if (regexec(&regExps->name.preg, name, 0, 0, 0) == 0
	&& ( strlen(regExps->args.expression) == 0
	     || regexec(&regExps->args.preg, args, 0, 0, 0) == 0)
	&& ( strlen(regExps->user.expression) == 0
	     || regexec(&regExps->user.preg, user, 0, 0, 0) == 0)
	&& ( strlen(regExps->group.expression) == 0
	     || regexec(&regExps->group.preg, group, 0, 0, 0) == 0)
	)
	return true;
...
static inline bool includeProcess(ProcessGroup aProcessGroup,
				  const char name[], const char args[],
				  const char user[], const char group[])
{
  std::vector<RegExpRule>::iterator regExps;
  
  for (regExps = aProcessGroup.includeProcs.begin();
       regExps != aProcessGroup.includeProcs.end();
       regExps++)
    if (regexec(&regExps->name.preg, name, 0, 0, 0) == 0
	&& ( strlen(regExps->args.expression) == 0
	     || regexec(&regExps->args.preg, args, 0, 0, 0) == 0)
...
void Analyzer::processRawData(ProcessList & processList, PerfData & rawData,
			      const bool fixTimes,
  ...
  throw(RunQError)
{
  ...
  try
      ...
      for (;;)
	{
	  ...
	  if (beforeTime((time_t*)&rawData.dynamicData.timeStamp,
	  ...
	  else if (!beforeTime((time_t*)&rawData.dynamicData.timeStamp,
			       endHour, endMinute))
	    {
	      break;
	    }
	  ...
	  for (processListIterator = rawData.dynamicData.processList.begin();
	       processListIterator != rawData.dynamicData.processList.end();
	       processListIterator++)
	    {
  ...
  for (processListIterator = initialProcesses.begin();
       processListIterator != initialProcesses.end();
       processListIterator++)
    {
      // Forget about badly formed names.
      // seems to happen sometimes on Solaris
...
void Analyzer::compileRegExps(DataStore * const logFile)
{
  std::vector<WorkLoad>::iterator	aWorkLoad;
  std::vector<ProcessGroup>::iterator	aProcessGroup;
  std::vector<RegExpRule>::iterator	iRegExps;
  std::vector<RegExpRule>::iterator	eRegExps;

  for (aWorkLoad = this->workLoads.begin();
       aWorkLoad != this->workLoads.end();
       aWorkLoad++)
    {
      if (logFile != 0)
	*logFile << "Workload " << aWorkLoad->name << endl;
      for (aProcessGroup = aWorkLoad->processGroups.begin();
	   aProcessGroup !=  aWorkLoad->processGroups.end();
	   aProcessGroup++)
	{
	  if (logFile != 0)
	    *logFile << "  Group " << aProcessGroup->name << endl;
	  for (iRegExps = aProcessGroup->includeProcs.begin();
	       iRegExps != aProcessGroup->includeProcs.end();
	       iRegExps++)
	    {
	      if (logFile != 0)
		*logFile << "    iexpression " << iRegExps->name.expression;
	      regcomp(&iRegExps->name.preg, iRegExps->name.expression,
		      REG_NOSUB);	
	      if (strlen(iRegExps->args.expression) > 0)
		{
		  if (logFile != 0)
		    *logFile << " + " << iRegExps->args.expression;
		  regcomp(&iRegExps->args.preg, iRegExps->args.expression,
			  REG_NOSUB);	
		}
	      if (strlen(iRegExps->user.expression) > 0)
		{
		  ...
		  regcomp(&iRegExps->user.preg, iRegExps->user.expression,
			  REG_NOSUB);	
		}
	      if (strlen(iRegExps->group.expression) > 0)
		{
		  ...
		  regcomp(&iRegExps->group.preg, iRegExps->group.expression,
			  REG_NOSUB);	
		}
	      if (logFile != 0)
		*logFile << endl;
	    }
	  for (eRegExps = aProcessGroup->excludeProcs.begin();
	       eRegExps != aProcessGroup->excludeProcs.end();
	       eRegExps++)
	    {
	      if (logFile != 0)
		*logFile << "    eexpression " << eRegExps->name.expression;
	      regcomp(&eRegExps->name.preg, eRegExps->name.expression,
		      REG_NOSUB);	
	      if (strlen(eRegExps->args.expression) > 0)
		{
		  if (logFile != 0)
		    *logFile << " + " << eRegExps->args.expression;
		  regcomp(&iRegExps->args.preg, eRegExps->args.expression,
			  REG_NOSUB);	
		}
	      if (strlen(eRegExps->user.expression) > 0)
		{
		  ...
		  regcomp(&eRegExps->user.preg, eRegExps->user.expression,
			  REG_NOSUB);	
		}
	      if (strlen(eRegExps->group.expression) > 0)
		{
		  ...
		  regcomp(&eRegExps->group.preg, eRegExps->group.expression,
			  REG_NOSUB);	
		}
	      if (logFile != 0)
		*logFile << endl;
...
void Analyzer::addProcessToGroup(PerfData & rawData,
...
bool Analyzer::findProcessGroup(WorkLoad *& theWorkLoad,
		      ProcessGroup *& theProcessGroup,
		      const char name[], const char args[],
		      const char user[], const char group[])
{
  ...
  for (workLoads = this->workLoads.begin();
    ...
    for (processGroups = workLoads->processGroups.begin();
	 ...
	 processGroups++)
	for (iRegExps = processGroups->includeProcs.begin();
	     iRegExps != processGroups->includeProcs.end();
	     iRegExps++)
...
void Analyzer::analyze(PerfData & rawData, const bool fixTimes,
		       DataStore * const logFile,
		       const int startHour, const int startMinute,
		       const int endHour, const int endMinute)
{
...
void Analyzer::report(std::ostream & output)
{
  std::vector<WorkLoad>::iterator workLoads;
  std::vector<ProcessGroup>::iterator	processGroups;
...
void Analyzer::analyze(const char name[], const bool fixTimes,
		       DataStore * logFile,
		       const int startHour, const int startMinute,
		       const int endHour, const int endMinute)
{
