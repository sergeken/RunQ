...
public:
    ...
    bool
    operator== (const ProcessID & rigth) const;
    bool
    operator!= (const ProcessID & rigth) const;
    bool
    operator< (const ProcessID & rigth) const;
...
inline bool
ProcessID::operator== (const ProcessID & rigth) const
{
...
struct ProcessID_lt {
    bool
    operator () (const ProcessID & first, const ProcessID & second) const
    {
        return first < second;
    }
};
...
public:
    ProcessList
    findChildren (const long PPID);
    ProcessData*
    findProcess (const ProcessID PID);
};
...
inline ProcessData*
ProcessList::findProcess (const ProcessID PID)
{
