...
DataStore::DataStore(const char name[], const std::_Ios_Openmode mode, const RunQFileType aFileType) : std::fstream (name, mode)
{
...
void
DataStore::get (void*data, const size_t size)
{
    read ((char*)data, size);
    if (!*this)
...
void
DataStore::put (void*data, const size_t size)
{
