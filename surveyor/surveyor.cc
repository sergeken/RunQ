...
WatchValues * Surveyor::watchProcess(const char name[], const char args[])
{
  vector<WatchValues>::iterator regExps;
  
  for (regExps = watchValues.begin();
       regExps != watchValues.end();
       regExps++)
...
static char * formatUserMessage(vector<Message> message)
{
  ...
  for (messageIter = message.begin();
       messageIter != message.end();
       messageIter++)
    {
...
void Surveyor::watchProcesses(void) throw(RunQError)
{
  ...
  for (;;) // Ever
	...
	syslog(LOG_DEBUG, "Mark");
      ...
      for (iter = previousProcesses.begin();
	   iter != previousProcesses.end();
	   iter++)
	{
  ...
  try
    {
      ...
      vector<WatchValues>::iterator iter;
      int option;
      ...
      if (yymain(argv[optind], &theSurveyor))
	{
      ...
      for (iter = theSurveyor.watchValues.begin();
	   iter != theSurveyor.watchValues.end();
	   iter++)
	{
	  vector<Message>::iterator messageIter;
	  ...
	  if (strlen(iter->args.expression) > 0)
	    ...
	    if (verbose)
	      fprintf(stderr, "scanning %s %s %f",
		     ...
		     fabs(iter->CPUUsage));
	  fprintf(stderr, " {");
	  for (messageIter = iter->message.begin();
	       messageIter != iter->message.end();
	       messageIter++)
	    {
	      switch (messageIter->type)
		{
